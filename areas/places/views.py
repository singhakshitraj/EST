...
def front(request):
    ...
    return render(request,'places/landing_page.html',{'property':properties})
...
def createProperty(request):
    ...
    return render(request=request,template_name='places/create_property.html',context={'form':form})

def viewProperty(request,id):
    prop = get_object_or_404(Property,pk=id)
    return render(request=request,template_name='places/property_page.html',context={'property':prop})
...
def purchaseProperty(request,id):
    if request.method == 'POST':
        ...
        return render(request,'places/property_page.html')
...
def search(request):
    if request.method == 'GET':
        ...
        return render(request,'places/search.html',context={'data':props})        
    else:
...
def confirm(request,id):
    ...
    if request.method == 'POST' and request.POST.get('full_name'):
        if item.open == False:
            ...
            item.save()
            return redirect('Front')
    return render(request,'places/confirm_buy.html',{'property':item})